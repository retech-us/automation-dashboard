...
Goal: fix failing tests, and update the spec if the app's intended behaviour changed. The full Heal workflow (find failures, debug one, apply the fix, reconcile with the spec, iteration) lives in [spec-driven-heal.md](spec-driven-heal.md).
