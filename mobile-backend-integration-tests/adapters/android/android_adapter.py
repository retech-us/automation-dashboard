"""
Android Headless Mobile Adapter.
Simulates Android JVM runtime execution of Retrofit2/OkHttp4 API layer and repositories.
"""
...
class AndroidMobileAdapter(BaseMobileAdapter):
    """
    Headless Adapter executing the exact contract of Android production components:
    - AuthApi (verifyCode, getUserInfo)
    - TaskApi (getTaskDetails)
    - Preferences / Token storage
    - AuthorizationInterceptor & Headers
    """

    def __init__(self):
        self._platform = PlatformType.ANDROID
        self._base_url = "https://epsilon.rebotics.net"
        self._build_variant = "production"
        self._custom_headers: Dict[str, str] = {}
        self._state = MobileState(platform=PlatformType.ANDROID)
    ...
    def _get_headers(self, requires_auth: bool = True) -> Dict[str, str]:
        ...
        headers = {
            ...
            "User-Agent": f"Store Intelligence {variant_name} Android/4.x (Headless; JVM)",
        }
    ...
    def _log_http_request(self, method: str, url: str, headers: Dict[str, str], body: Optional[str] = None):
        print(f"\n      [Android Network] >>> {method} {url}")
        print(f"      [Android Network] >>> Headers: {json.dumps(headers)}")
        if body:
            print(f"      [Android Network] >>> Body: {body}")

    def _log_http_response(self, status_code: int, server: Optional[str], body: str, duration_ms: float):
        print(f"      [Android Network] <<< HTTP {status_code} ({server or 'Real Backend'}) in {duration_ms:.1f}ms")
        print(f"      [Android Network] <<< Response: {body[:300]}")

    def resolve_instance_host(self, company: str, gateway_url: Optional[str] = None) -> str:
        """
        Executes production ServerInstanceApi / AuthorizationInstanceInteractor:
        ...
        try:
            ...
            print(f"      [Android Network] ⚠️ Instance resolution error: {e}")
            raise
    ...
    def upload_image(self, upload_url: Optional[str] = None, image_bytes: Optional[bytes] = None) -> MobileState:
        ...
        if not image_bytes:
            raise ValueError("[Android Adapter Strict] Missing image_bytes for S3 upload. Dummy bytes are strictly rejected.")
        ...
        try:
            ...
            raise IOError(f"[Android Adapter Strict] S3 Upload failed: {e}") from e
        return self.get_state()
    ...
    def create_task_definition(
        ...
        title: str = "Intelligent Reset Automated Live Run",
    ) -> Dict[str, Any]:
    ...
    def upload_and_create_bay_scan(
        ...
        try:
            ...
            if s3_url and fields:
                ...
                if img_data is None:
                    ...
                    if img_path.exists():
                        ...
                        raise FileNotFoundError(f"[Android Adapter Strict] Real scan photo for Bay '{section_name}' not found at {img_path}. Dummy scans are strictly prohibited.")
                ...
                try:
                    ...
                    self._record_action(ActionType.ERROR_OCCURRED, payload={"error": self._state.last_error})
                    raise IOError(f"[Android Adapter Strict] S3 Upload for Bay '{section_name}' failed: {s3_err}") from s3_err
            ...
            act_payload = {
                ...
                "client_model": "Headless Test Device",
                "client_type": "phone",
