...
const BUILD_TAG = '20260805b';
const DASHBOARD_VERSION = window.DASHBOARD_VERSION || '12';
...
const DEMO_STEPS = [
  { id: 'overall-banner', title: 'Release confidence', tip: 'Start here — one score anyone can read in a meeting.' },
  { id: 'business-areas', title: 'Product areas', tip: 'Traffic lights map tests to journeys the business cares about.' },
  ...
  { id: 'repo-cards', title: 'Suite health', tip: 'Web, iOS, Android, and API at a glance.' },
  { id: 'failures-panel', title: 'What needs attention', tip: 'Plain-language issues — no need to open Allure first.' },
];
...
const CATEGORY_NEXT_STEP = {
  ...
  timeout: 'Check page/API latency and whether a deploy is in progress.',
  ...
  environment: 'Check environment health and recent deploys before retesting.',
  ...
  return `
    <div class="failure-item">
      <div class="failure-item__tags">
        ...
        <span class="category-tag">${escapeHtml(explanation.categoryLabel)}</span>
        <span class="suite-tag">${escapeHtml(repoLabel(f.repo))}</span>
      </div>
      <div class="failure-item__body">
        <h4>${escapeHtml(friendlyFailureTitle(f))}</h4>
        <p class="failure-meaning"><strong>What it means:</strong> ${escapeHtml(explanation.meaning)}</p>
...
function parseCountReason(reason) {
  ...
  return parts.length ? parts.join(' and ') + ' in this journey' : null;
}
...
function explainFailure(failure) {
  ...
  const countPhrase = parseCountReason(failure.reason);
  ...
  if (rawReason) {
    ...
    meaning = `${title} on ${suite} has ${countPhrase}. Likely cause: ${categoryLabel.toLowerCase()}.`;
  } else if (failure.status === 'broken') {
    meaning = `${title} on ${suite} is unstable right now (${categoryLabel.toLowerCase()}).`;
  } else {
    meaning = `${title} on ${suite} did not meet expectations (${categoryLabel.toLowerCase()}).`;
  }

  return {
    categoryLabel,
    meaning,
    nextStep: CATEGORY_NEXT_STEP[category] || CATEGORY_NEXT_STEP.unknown,
