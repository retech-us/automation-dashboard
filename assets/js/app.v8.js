...
const BUILD_TAG = '20260625d';
const DASHBOARD_VERSION = window.DASHBOARD_VERSION || '8';
...
function renderCardFailures(summary) {
  ...
  const items = summary.topFailures.slice(0, 2).map((f) => `
  ...
  return `
    <article class="card card--${config.id} card--${status}" data-repo="${config.id}">
      ...
      <div class="card__body">
        ...
        <ul class="meta-list meta-list--grid">
          ...
          ${metaItem('Data source', summary?.dataSource)}
        </ul>
