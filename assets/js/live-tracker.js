  ...
  class LiveTracker {
    ...
    renderLiveBanner() {
      ...
      if (this.activeRuns.size === 0) {
        ...
        if (statusPill) statusPill.textContent = 'Idle · Listening to CI';
        if (contentEl) {
          contentEl.innerHTML = `
            <div class="live-empty-state" style="text-align:center;padding:32px 16px;">
              ...
              <div class="live-empty-meta" style="margin-top:24px;font-size:12px;color:var(--text-secondary,#94a3b8);">
                <span class="live-pulse" style="display:inline-block;width:8px;height:8px;"></span> Real-time monitoring active · Auto-checks every 5s
              </div>
            </div>
          `;
        }
