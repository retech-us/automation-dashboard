...
const BUILD_TAG = '20260805e';
const DASHBOARD_VERSION = window.DASHBOARD_VERSION || '15';
...
const DEMO_STEPS = [
  ...
  { id: 'trends-panel', title: 'Automation trends', tip: 'Pass-rate graph — switch Daily / Weekly / Monthly / Quarterly / Yearly.' },
...
function bucketLabel(key, bucket) {
  ...
  if (bucket === 'week') return key.replace('-W', ' W');
  if (bucket === 'month') {
    const [y, m] = key.split('-');
...
function collectTrendPoints(history, liveResults) {
  const points = [...(history?.points || [])];
  // Merge latest live suite scores as today/run-day points
...
function aggregateTrendSeries(points, range) {
  ...
  const buckets = new Map(); // key -> { suite -> {sum, weight} }

  for (const p of filtered) {
    const day = parseDay(p.date);
    const key = bucketKey(day, range.bucket);
    if (!buckets.has(key)) buckets.set(key, { key, order: day.getTime(), suites: {} });
    const slot = buckets.get(key);
    const weight = Math.max(1, p.total || 1);
    const cur = slot.suites[p.suite] || { sum: 0, weight: 0 };
  ...
  // Overall weighted across suites present in each bucket
  ...
  return { labels, series, estimated, pointCount: filtered.length };
}

function buildTrendPath(values, width, height, padX, padY) {
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    const y = padY + (1 - values[i] / 100) * usableH;
...
function renderTrendChartSvg(agg) {
  ...
  const padY = 28;
  const lines = [
    { id: 'overall', label: 'Overall', color: '#e2e8f0', width: 3 },
    ...REPO_CONFIG.map((c) => ({
      id: c.id,
  ...
  const grid = [0, 25, 50, 75, 100].map((pct) => {
    ...
    return `<line x1="${padX}" y1="${y}" x2="${width - padX}" y2="${y}" class="chart-grid" />
      <text x="${padX - 8}" y="${y + 4}" class="chart-axis">${pct}%</text>`;
  }).join('');

  const labelStep = Math.max(1, Math.ceil(agg.labels.length / 8));
  ...
  const paths = lines.map((line) => {
    const { d, pts } = buildTrendPath(agg.series[line.id] || [], width, height, padX, padY);
    if (!d) return '';
    const dots = pts.map((p) => `
      <circle cx="${p.x}" cy="${p.y}" r="3.5" fill="${line.color}">
        <title>${escapeHtml(line.label)}: ${p.v}% · ${escapeHtml(agg.labels[p.i] || '')}</title>
      </circle>`).join('');
    return `<path d="${d}" fill="none" stroke="${line.color}" stroke-width="${line.width}" stroke-linecap="round" stroke-linejoin="round" />${dots}`;
  }).join('');

  const legend = lines.map((line) => `
    <span class="chart-legend__item"><i style="background:${line.color}"></i>${escapeHtml(line.label)}</span>
  `).join('');
...
function renderTrendsPanel(summaries) {
  ...
  const mini = REPO_CONFIG.map((cfg, i) => {
  ...
  return `
    <div class="panel__header trend-panel__header">
      <div>
        ...
        <p>Pass rate over time — default Weekly. Switch range as needed.</p>
      </div>
    ...
    <p class="chart-footnote">
      ...
      · ${agg.pointCount} raw points
      ${agg.estimated ? ' · includes estimated backfill until denser CI history accumulates' : ''}
    </p>
...
function setTrendRange(rangeId) {
  ...
  if (CURRENT_RESULTS.length) {
    ...
    if (trendsEl) trendsEl.innerHTML = renderTrendsPanel(CURRENT_RESULTS);
  }
}
...
function answerFromKnowledge(question) {
  ...
  if (/overall|release confidence|are we ready|release ready|health score|summary/.test(q) && !suiteId) {
    return [
      `Overall release confidence is **${k.overall.label}** (${k.overall.score}/100, ${k.overall.passPct}% pass).`,
      k.overall.sentence,
      `Checks: ${k.overall.totalPassed} passed, ${k.overall.totalFailed} need review, ${k.overall.totalTests} total.`,
